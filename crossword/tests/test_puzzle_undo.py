...
class TestPuzzleUndo:
    ...
    def test_undo_text(self):
        puzzle = self.create_test_puzzle()
        assert 'RIOT' == puzzle.get_text(10, 'A')
        assert 'ERO ICA ' == puzzle.get_text(4, 'D')
        assert [] == puzzle.undo_stack
        assert [] == puzzle.redo_stack

        puzzle.set_text(10, 'A', 'ZOOM')
        assert 'ZOOM' == puzzle.get_text(10, 'A')
        assert 'EZO ICA ' == puzzle.get_text(4, 'D')
        assert [
            ['text', 10, 'A', 'RIOT']
        ] == puzzle.undo_stack
        assert [] == puzzle.redo_stack
        ...
        assert 'PLUS' == puzzle.get_text(10, 'A')
        assert 'EPO ICA ' == puzzle.get_text(4, 'D')
        assert [
            ['text', 10, 'A', 'RIOT'],
            ['text', 10, 'A', 'ZOOM']
        ] == puzzle.undo_stack
    ...
    def test_to_json(self):
        ...
        assert 'RIOT' == puzzle.get_text(10, 'A')
        assert None == puzzle.get_clue(10, 'A')
        assert [] == puzzle.undo_stack
        assert [] == puzzle.redo_stack
