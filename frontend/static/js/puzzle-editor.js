...
async function do_puzzle_import_ipuz() {
    const fileInput = document.getElementById('ipuz-file-input');
    fileInput.value = '';
    fileInput.onchange = async (evt) => {
        const file = evt.target.files[0];
        if (!file) return;

        const defaultName = file.name.replace(/\.(ipuz|json)$/i, '');
        ...
        inputBox(
            ...
            async (name) => {
                ...
                try {
                    const data = await apiFetch('POST', '/api/import/ipuz', { name, content });
                    if (data.error) { showMessageLine(`Import failed: ${data.error}`, 'error', 0); return; }
                    showMessageLine(`Imported "${name}" — opening…`, 'notice');
                    await _openPuzzleInEditor(name);
                } catch (e) {
...
async function _downloadExport(name, format) {
    ...
    const filenameMap = { puz: `${name}.txt`, puzbin: `${name}.puz`, xd: `${name}.xd`, ipuz: `${name}.ipuz`, xml: `${name}.xml`, nyt: `nytimes_${name}.pdf`, solver: `${name}.pdf`, solved: `${name}-solution.pdf` };
    const labelMap  = { puz: 'Across Lite', puzbin: '.puz Binary', xd: 'xword xd', ipuz: 'ipuz', xml: 'Crossword Compiler XML', nyt: 'New York Times', solver: 'Solver PDF', solved: 'Solution PDF' };
