"""Tests for the puzzle-save-comments migration tool
(tools/dev/migrate_puzzle_save_comments.py)."""
...
TOOL_PATH = (
    Path(__file__).resolve().parents[2] / "tools" / "dev" / "migrate_puzzle_save_comments.py"
)

# The pre-comment-column schema this tool migrates *from* — already has
# `content` (unlike the source schema for migrate_puzzle_content_snapshots.py).
NO_COMMENT_SCHEMA = """
...
def load_tool():
    ...
    spec = importlib.util.spec_from_file_location("migrate_puzzle_save_comments", TOOL_PATH)
...
@pytest.fixture
def old_db(tmp_path):
    """Build a no-comment-column database with a puzzle that has several history rows,
    some with a content snapshot and some without (predating content snapshots)."""
    path = tmp_path / "old.db"
    conn = sqlite3.connect(path)
    conn.executescript(NO_COMMENT_SCHEMA)
    ...
    conn.executemany(
        ...
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [
            (1, 1, "draft", None, None, None, None, "2026-01-01T00:00:00"),
            (2, 1, "finished", None, None, None, solved, "2026-06-01T00:00:00"),
            (3, 1, "submitted", "NYT", "2026-06-06", None, solved, "2026-06-06T00:00:00"),
            (4, 2, "draft", None, None, None, empty, "2026-01-01T00:00:00"),
        ],
    )
    conn.commit()
    conn.close()
...
def test_new_database_has_comment_column(migrate, tmp_path):
...
def test_end_to_end_migration(migrate, old_db, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["migrate_puzzle_save_comments.py", "--old-dbfile", str(old_db),
    ...
    # Existing content snapshots are preserved, not dropped.
    assert history[1]["content"] is None
    assert history[2]["content"] == TestPuzzle.create_solved_atlantic_puzzle().to_json()
    assert history[4]["content"] == TestPuzzle.create_atlantic_puzzle().to_json()

    # Nothing to backfill — every copied row has a NULL comment.
    assert all(row["comment"] is None for row in history.values())
