"""
Rebuild the crossword database into a new file with a `content` column on
...
column: `puzzle_state_history.content`, holding a full copy of the
puzzle's JSON content as it stood at the moment each row was written.

Unlike migrate_puzzle_state_history.py, this migration doesn't need to
filter out working copies or synthesize anything — it's a straight copy of
both tables (ids preserved) with one column added. Existing history rows
are copied across with `content` left NULL: there is nothing to backfill,
since the old schema never captured content, so there is no way to
reconstruct what a puzzle looked like at any past save before this
migration ships. From this point on, every save writes its own snapshot —
see docs/dev/puzzle_content_snapshots.md.
...
(see docs/dev/puzzle_content_snapshots.md), which remains the source of
truth for the running app. Keep them in sync by hand.

Usage:
    python3 tools/dev/migrate_puzzle_content_snapshots.py --new-dbfile PATH [--old-dbfile PATH] [--dry-run]
...
def main() -> None:
    """Read the old DB and rebuild it into a new file with a content column on puzzle_state_history."""
...
def fetch_history(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    """Return every row of the puzzle_state_history table, unfiltered."""
    cur = conn.cursor()
    cur.execute(
        "SELECT id, puzzle_id, state, publisher, date_submitted, date_published, changed_at"
...
def copy_data(conn: sqlite3.Connection, puzzles: list[sqlite3.Row], history: list[sqlite3.Row]) -> None:
    """Insert the puzzle and history rows into the new DB, preserving ids. `content` is left NULL."""
    conn.executemany(
        "INSERT INTO puzzles"
    ...
    conn.executemany(
        ...
        [
            (row["id"], row["puzzle_id"], row["state"], row["publisher"],
             row["date_submitted"], row["date_published"], row["changed_at"])
            for row in history
        ],
    )
